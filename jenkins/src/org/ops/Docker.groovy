...
def build() {
    ...
    def cmdMap = [
            ...
            dotnet: {
                ...
                return StringUtils.format(
                        "rm -rf {0}/build && dotnet publish --configfile {1} -c Release -p:PublishSingleFile=true,IncludeNativeLibrariesForSelfExtract=true --self-contained false {0} -o {0}/build",
                        env.param_project_root,
                        "${CONFIG_FILE_NUGET}"
                )
            },
