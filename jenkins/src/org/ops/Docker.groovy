...
def deploy() {
    ...
    configFileProvider([
        ...
        cmd+="DOCKER_HOST=ssh://opsor@${env.param_docker_deploy_host} docker-compose --project-name ${env.param_release_name} --file ${docker_compose_file} --env-file ${param_file} up --detach --remove-orphans || echo \$SSH_AGENT_PID"
        try{
            sh "${cmd}"
        }catch(e){
            sh "kill ${e};exit 2"
