...
def build() {
    ...
    configFileProvider([configFile(fileId: "docker-config", targetLocation: 'docker-config/config.json')]) {
        sh "docker --config docker-config push ${env.param_release_name}:${env.param_release_version}"
    }
}
