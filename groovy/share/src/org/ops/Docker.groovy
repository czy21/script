...
def prepare(Map map){
    ...
    env.param_image_name=Arrays.stream(new String[]{env.param_registry_repo,env.param_registry_dir,env.param_project_module}).filter(t -> t != null).collect(Collectors.joining("/"))
