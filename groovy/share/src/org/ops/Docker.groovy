...
def build() {
    switch (env.param_code_type) {
        case "java":
            ...
            gradle_cmd = ["clean", "build"].collect { t -> [env.param_project_module, t].findAll { c -> !(StringUtils.isEmpty(c) || c.equals("null")) }.join(":") }.join(" ")
    ...
    sh "docker build --tag ${env.param_release_name}:${env.param_release_version} --file ${env.param_docker_file} ${env.param_project_context} --force-rm"
