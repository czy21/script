...
def prepare(Map map){
    ...
    env.param_image_name=[env.param_registry_repo,env.param_registry_dir,[env.param_project_name,env.param_project_module].findAll{it!=""}.join("-")].join("/")
    env.param_docker_file = [env.param_project_root,env.param_project_module,"Dockerfile"].findAll{it!=""}.join("/")
    env.param_docker_file_context = [env.param_project_root,env.param_project_module].findAll{it!=""}.join("/")
}
