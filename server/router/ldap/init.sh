...
db_ldif_etc_file="${ldap_etc_path}/slapd.d/cn=config/olcDatabase={1}mdb.ldif"
if [ "install" == "${param_command}" ];then
  ...
  sed -i 's|^olcDbDirectory.*|olcDbDirectory: /volume1/storage/docker-data/openldap/data|g' ${db_ldif_etc_file}
