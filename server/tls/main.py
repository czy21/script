...
class TLSRole(share.AbstractRole):
...
def get_cmds(role: TLSRole) -> list[str]:
    _cmds = []
    if role.role_init_sh.exists():
        _cmds.append("sh {}".format(role.role_init_sh.as_posix()))
    return _cmds


if __name__ == '__main__':
    share.Installer(pathlib.Path(__file__).parent, TLSRole).run()
