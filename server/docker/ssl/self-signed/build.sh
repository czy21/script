...
openssl genrsa -out ${tmp_generate}/server.key 4096 && openssl req -new -key ${tmp_generate}/server.key -out ${tmp_generate}/server.csr -config ${openssl_cnf} -nodes -subj "/C=CN/ST=SH/L=SH/O=Home/OU=IT/CN=server.cluster.com"
