...
class DockerRole(server.AbstractRole):

    def __init__(self, context: server.RoleContext) -> None:
        super().__init__(context)

        self.container_compose = "podman compose" if self.context.role_env.get("param_container_engine") == 'podman' else f"{self.gen_sudo()} docker compose"
        ...
        self.role_node_conf_path = self.context.role_node_path.joinpath("conf")
        self.role_node_deploy_file = self.context.role_node_path.joinpath("compose.yml")
    ...
    def compose_cmd(self, project_name, option):
        ...
        if self.role_node_deploy_file.exists():
            role_compose_files.append(self.role_node_deploy_file)
        cmd = [
    ...
    def install(self) -> list[str]:
        _cmds = []
        if self.role_conf_path.exists() or self.role_node_conf_path.exists():
    ...
    def build(self) -> list[str]:
        ...
        if self.context.args.target == "doc":
            ...
            self.role_doc_content = template_util.Template(file_util.read_text(self.root_doc_template_file)).render(**md_param)
        return _cmds
    ...
    def delete(self) -> list[str]:
        _cmds = []
        if self.role_compose_file.exists():
            _cmds.append(self.compose_cmd(self.role_project_name, "down --remove-orphans"))
        return _cmds
...
if __name__ == '__main__':
    server.Installer(pathlib.Path(__file__).parent, role_impl=DockerRole, role_deep=2).run()
