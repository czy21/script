...
def invoke(role_title: str, role_path: Path, **kwargs):
    ...
    env_dict["param_project_path"] = project_path
    conf_file = role_path.joinpath("conf")
    exec_file = role_path.joinpath("exec.sh")
    backup_file = role_path.joinpath("backup.py")
    ...
    if args.a == "backup":
        _cmds.append("python -B {0}".format(backup_file.as_posix()))
...
if __name__ == '__main__':
    ...
    share.execute(selected_option["list"], invoke, project_path=project_path, common_sh=common_sh, args=args)
