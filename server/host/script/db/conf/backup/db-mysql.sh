...
CLI="docker run --rm -i mysql:8.0.43"
...
function backup(){
  ...
  databases=$(${CLI} mysql --default-character-set=utf8mb4 -h$host -p$port -u$DB_USERNAME -p$DB_PASSWORD --skip-column-names -s -e 'SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME NOT IN ("mysql", "performance_schema","information_schema","sys") ORDER BY `SCHEMA_NAME` ASC' | xargs)
