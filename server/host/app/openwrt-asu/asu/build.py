...
def _build(build_request: BuildRequest, job=None):
        ...
        mounts.append(
            {
                "type": "bind",
                "source": str(apk_key),
                "target": "/builder/keys/" + "local-private-key.pem",
                "read_only": True,
            },
        )
        mounts.append(
            {
                "type": "bind",
                "source": str(apk_pub),
                "target": "/builder/keys/" + "local-public-key.pem",
                "read_only": True,
            },
        )
        mounts.append(
            {
                "type": "bind",
                "source": str(apk_pub),
                "target": "/builder/files/etc/apk/keys/openwrt-plugin.pem",
                "read_only": True,
            },
        )
    ...
    if build_request.repositories:
        ...
        repositories = ""
        if use_apk:
            repositories = "\n".join(build_request.repositories.values())

            (bin_dir / "repositories").write_text(repositories)
