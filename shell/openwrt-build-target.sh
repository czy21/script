...
if [ "$build" = true ];then
  echo '===== Build ====='
  [ "$prebuilt" = true ] && make tools/install toolchain/install -j$(nproc) BUILD_LOG=1

  rm -rf bin;make CONFIG_BUILDBOT=y CONFIG_IB=y CONFIG_IB_STANDALONE=y -j$(nproc) BUILD_LOG=1 V=s > make.log
fi
