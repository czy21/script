...
with pd.ExcelFile(path_or_buffer="3W.xlsx") as f:
    table = mongoClient["spi_local"]["ent_file_column_mapping"]
    sd_mapping_list = table.find_one({"businessType": "SD"})["fields"]
    sd_mapping_dict = dict(("*" + t["header"] if t.get("validators") is not None and any([p.get("required") for p in t.get("validators")]) else t["header"],
                            t["column"]
                            )
